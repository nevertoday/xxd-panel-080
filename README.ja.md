...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 080 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

一般的な商業イラストは、写真を滑らかなテンプレートへ置き換えたり、植物、記号、背景で画面を埋めたりしがちで、写真が本来持つ感情と潜在的な比喩を読み取れていません。

**Panel 080** は選択的記憶によって、被写体、構造、姿勢、物語上の関係、そして写真から自然に生まれる繊細な比喩を抽出します。それらを柔らかな有機幾何、輪郭線のないフラットな色面、デジタルガッシュ／クレヨン、粒状テクスチャで再構成します。セージグリーンとオリーブグリーンが植物系の色基調をつくり、少量の暖色が感情を照らし、広い余白が空気、時間、距離、沈黙を担います。

### 向いている用途

- 尺度、距離、正負形、余白によって、感情を目に見える画面関係へ変えたい場合
- 植物、道、反復形、遮蔽、遠くの小物から、新しい物語を押し付けず自然な比喩を抽出したい場合
- 有機幾何、粒状感、セージグリーン、少量の暖色でつくるデジタルガッシュと植物系配色を好む場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 080 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 柔らかな有機幾何 · デジタルガッシュとクレヨン · 粒子感 · 植物系配色 · 原写真由来の比喩 · 感情的余白
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 静かで、優しく、生命力があり、成熟した構図と芸術出版感を備えた感情のポスター |
| **識別しやすい特徴** | 柔らかな有機色面、粒状のデジタルガッシュ、低彩度の植物系配色、物語を担う広い余白 |
| **入力の尊重** | 被写体の同一性と物語上の関係を守り、比喩と全体の色温度を元写真から導いて新しい物語を押し付けない |
...
```text
同一性・構造・姿勢・物語関係・核となる感情を抽出 → 元写真から自然に生まれる繊細な比喩を一つ見つける → 柔らかな有機幾何と輪郭線のない色面で要約 → デジタルガッシュ、クレヨン、粒状の手仕事感を加える → 植物系の低彩度配色で元写真の特徴色を再調整 → 余白に空気・時間・想像を担わせる → 少量の文字を感情の脚注にする
```

## 完成作の特徴

- 被写体は柔らかな有機幾何、大きなシルエット、輪郭線のない色面で要約し、現実的な遠近法と写実的な光影を弱める。
- デジタルガッシュ、クレヨン跡、粒状ノイズが本物の手仕事の温度を与え、滑らかなベクター感を避ける。
- セージグリーンとオリーブグリーンを主に、クリームイエロー、グレーブルー、中性色を補助とし、少量のコーラル、アプリコット、マスタードで感情を照らす。
- 被写体は縮小、偏置、裁ち落とし、孤立配置ができ、余白が空気、時間、距離、沈黙、想像を能動的に担う。
- 比喩は写真から自然に生まれなければならず、文字は軽く、小さく、抑制された観察または感情の脚注にとどめる。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-080-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 080 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-080 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-080 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-080 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-080 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 080 を適用し、共通設定は一度だけ解決
/xxd-panel-080 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-080`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-080-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-080-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全プロジェクト一覧

80 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 080 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–080） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
