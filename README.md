<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 080 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 080 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
提取身份、结构、姿态、叙事关系与核心情绪 → 找到一个自然源于原图的微妙隐喻 → 以柔性有机几何和无描边色块概括 → 加入数字水粉、蜡笔和颗粒手作感 → 用植物系低饱和配色重调原图特征色 → 让留白承担空气、时间与想象 → 少量文字成为情绪注脚
```

## 成品中最容易识别的风格特点

- 主体以柔性有机几何、大面积剪影与无描边色块概括，弱化真实透视和写实光影。
- 数字水粉、蜡笔痕迹与颗粒噪点带来真实手作温度，避免光滑矢量感。
- 配色以鼠尾草绿、橄榄绿为主，辅以奶油黄、灰蓝和中性色，小面积珊瑚粉、杏橙或芥末黄点亮情绪。
- 主体可以缩小、偏置、裁切或孤立放置，留白主动承担空气、时间、距离、沉默与想象。
- 隐喻必须自然来自照片；文字只做轻、小、克制的观察或情绪注脚。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-080-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–080） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
