...
# 🦁 XXD Panel 080｜유기적 과슈의 장

### 사진의 감정과 은유를 식물색 디지털 과슈와 시적 여백으로 재구성합니다
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 080이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

흔한 상업 일러스트는 사진을 매끈한 템플릿으로 바꾸거나 식물, 기호와 배경으로 화면을 가득 채우면서, 사진이 지닌 감정과 잠재적 은유를 제대로 읽지 못하기 쉽습니다.

**Panel 080**는 선택적 기억으로 피사체, 구조, 자세, 서사 관계와 사진에서 자연스럽게 나온 하나의 섬세한 은유를 추출합니다. 이를 유연한 유기 기하, 윤곽선 없는 평면 색면, 디지털 구아슈／크레용과 입자 질감으로 재구성합니다. 세이지 그린과 올리브 그린이 식물계 색조의 바탕을 만들고, 소량의 따뜻한 색이 감정을 밝히며, 넓은 여백이 공기, 시간, 거리와 침묵을 담당합니다.

### 이런 경우에 적합합니다

- 크기, 거리, 포지티브·네거티브 형태와 여백으로 감정을 눈에 보이는 화면 관계로 만들고 싶을 때
- 식물, 길, 반복 형태, 가림이나 멀리 있는 작은 사물에서 새 이야기를 강요하지 않고 자연스러운 은유를 추출하고 싶을 때
- 유기 기하, 입자 질감, 세이지 그린과 절제된 따뜻한 색이 만드는 디지털 구아슈와 식물계 배색을 좋아할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 080로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 부드러운 유기 기하 · 디지털 과슈와 크레용 · 입자 질감 · 식물계 색상 · 원본에서 나온 은유 · 감정의 여백
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 고요하고 부드럽고 생명력 있으며, 성숙한 구성과 예술 출판 감각을 지닌 감정 포스터 |
| **대표 특징** | 유연한 유기 색면, 입자성 디지털 구아슈, 저채도 식물계 배색과 서사를 담당하는 넓은 여백 |
| **입력 존중 방식** | 피사체의 정체성과 서사 관계를 보존하고, 은유와 전체 색온도를 원본에서 가져와 새 이야기를 강요하지 않음 |
...
```text
정체성·구조·자세·서사 관계·핵심 감정 추출 → 원본에서 자연스럽게 나온 하나의 섬세한 은유 찾기 → 유연한 유기 기하와 윤곽선 없는 색면으로 요약 → 디지털 구아슈·크레용·입자성 손작업 감각 추가 → 식물계 저채도 배색으로 원본 특징색 재조율 → 여백이 공기·시간·상상을 담당 → 소량의 글자를 감정의 각주로 사용
```

## 완성작의 특징

- 피사체는 유연한 유기 기하, 넓은 실루엣과 윤곽선 없는 색면으로 요약하며 실제 원근과 사실적 빛·그림자를 약화합니다.
- 디지털 구아슈, 크레용 흔적과 입자 노이즈가 실제 손작업의 온기를 더해 매끈한 벡터 느낌을 피합니다.
- 세이지 그린과 올리브 그린을 중심으로 크림 옐로, 그레이 블루, 중성색을 보조하고, 소량의 코럴·애프리콧·머스터드 색으로 감정을 밝힙니다.
- 피사체는 축소, 치우친 배치, 크롭 또는 고립 배치가 가능하며 여백이 공기, 시간, 거리, 침묵과 상상을 능동적으로 담당합니다.
- 은유는 사진에서 자연스럽게 나와야 하며, 글자는 가볍고 작고 절제된 관찰 또는 감정의 각주로만 사용합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-080-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 080의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-080 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-080 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-080 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-080 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-080 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-080 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 080 적용, 공통 설정은 한 번만 확정
/xxd-panel-080 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-080`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-080-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-080-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트 목록

80개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 080까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–080) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
